...
import type { LayoutMode } from "../lib/dayTable";
  ...
  return (
    <>
      <div className="mb-7 flex flex-wrap items-center justify-between gap-2">
        ...
        <div
          className={cn(
            "flex items-center gap-2.5 rounded-xl border border-border bg-muted px-4 py-2",
            isTable && "invisible"
          )}
        >
          <span
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="flex flex-1 items-center justify-center gap-3.5 md:flex-none md:justify-start">
          {isTable ? (
            <button
              type="button"
              onClick={goToTableToday}
              className={cn(toggleVariants({ variant: "outline" }))}
            >
              Сегодня
            </button>
          ) : (
        ...
        >
          <ToggleGroupItem value="grid" aria-label="Вид: сетка">
            <LayoutGrid className="size-4" />
          </ToggleGroupItem>

          <ToggleGroupItem value="table" aria-label="Вид: таблица">
